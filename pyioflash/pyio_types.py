...
class GeometryData(_BaseData):
    ...
    #blk_dict_x: Dict[float, int] = field(repr=False, init=False, compare=False) # will depreciate flattening of data
    ...
    @staticmethod
    def _get_neighbors(id, blocks, blk_tree_str, dirs):
        nbr_inflg = -1
        neighbors = []
        for dir in dirs:
            if blocks[dir] >= 0:
                neighbors.append((0, blocks[dir]))
            elif blocks[dir] == nbr_inflg:
                neighbors.append((1, blk_tree_str[blocks[4]][dir]))
            else:
                neighbors.append((2, blocks[dir]))
        return neighbors
    ...
    def _init_process(self, file, code, form) -> None:        
        ...
        self.blk_neighbors = [GeometryData._get_neighbors(id, blocks, self.blk_tree_str, 
                                                          range(int(2 * self.grd_dim))) 
                              for id, blocks in enumerate(self.blk_tree_str)]
...
class FieldData(_BaseData):
    ...
    def _fill_guard_data(data, geometry):
        ...
        for block, neighbors in enumerate(geometry.blk_neighbors):
            ...
            #data[block, :, -1, -1] = 0.5 * (data[block, :, -2, -1] + data[block, :, -1, -2])    
